...
Approving writes canonical data to Postgres. It does not change the site: the site is a
static build from `data/*.json`, generated by `node build.js` with no database access at
all. **Publish to site** is the step that connects the two.

```
approve  →  Postgres  →  Publish  →  data/*.json  →  one commit  →  Vercel  →  the site
```

Pressing it runs `scripts/export-data.js` against the database, validates the result,
...
- **Only changed files.** Change detection is git's own object id, computed locally, so
  an unchanged file costs nothing and produces no diff noise.
- **It validates first.** Every `src` id in the exported files has to resolve against
  `sources.json` — the one integrity failure that is invisible downstream, since a stat
  with an unresolvable source renders silently with no footnote. If the export does not
  pass, nothing is committed.
...
This is also the moment `data/*.json` becomes generated rather than hand-written. Editing
those files by hand still works, but the next publish overwrites them from the database —
so once publishing is set up, the desk is where data changes.
