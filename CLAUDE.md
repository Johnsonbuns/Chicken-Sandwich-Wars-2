...
- **Minnesota CARDS — YES, and this is the single most productive source on the list.** `www.cards.commerce.state.mn.us` publishes whole FDD PDFs, free, no account. **The `www.` is not optional: `cards.commerce.state.mn.us` is NXDOMAIN.** This file recorded it as "returns 403 here" for months and a later run read the resulting proxy error as a policy block; both were wrong, and on the strength of that five AUVs were written up as unobtainable and left overdue for twenty months. They were always one subdomain away. Treat a "blocked" note in this file as a hypothesis, not a finding — re-test it before you plan around it.
  `scripts/fdd-fetch.py` drives it: `--list "<brand>"` shows every filing, `--get "<brand>" --year <yyyy>` downloads the `Marked FDD`. Two mechanics matter — the row-number cell is a `<th>` so a `<td>`-only parse shifts every column by one, and the download GUID arrives in literal curly braces that must be percent-encoded or the URL 400s. As of 2026-09-02, Marked FDDs are on file for Bojangles (7 years), Dave's Hot Chicken (7), Slim Chickens (6), Chicken Salad Chick (5), Popeyes (4), Pollo Campero (4) and Zaxby's (2). Golden Chick has none. **Multiple years per brand means the longitudinal roster comparison is buildable today.**
  CARDS lags state registration dates: a franchisor registered in Wisconsin as of 2026-04 may still only have its 2025 edition here. That is normal — the older edition is the primary source for its own fiscal year.
