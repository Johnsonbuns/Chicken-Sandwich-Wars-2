...
/* How many records a data file holds. The thirteen files are either an array of records
   or an object of named groups (sources keyed by id, movement's openings and closures),
   so both shapes are counted rather than assuming one. */
  ...
  try {
    ...
    if (v && typeof v === 'object') {
      return Object.values(v).reduce(
        (n, x) => n + (Array.isArray(x) ? x.length : 1), 0);
    }
    return null;
  } catch { return null; }
}
...
const OPS = {
  ...
  async publish(a, token) {
    ...
    /* 4. Refuse a publish that would gut the site.
     *
     * This overwrites data/ with whatever the database holds, so a database that is
     * incomplete — a half-applied seed, a migration run against the wrong project, a
     * table truncated by accident — publishes as deletion. Every other check here passes
     * happily on a file with three brands in it.
     *
     * Deleting records is legitimate, so this is not a ban: it is a stop when a file
     * loses more than a tenth of its records, and it can be overridden by someone who
     * has read what it says.
     */
    const counts = [];
    for (const [name, body] of changed) {
      const sha = existing.get(`data/${name}`);
      if (!sha) { counts.push({ name, before: null, after: recordCount(body) }); continue; }
      const blob = await gh(`/repos/${e.owner}/${e.repo}/git/blobs/${sha}`, { token: e.token });
      const before = recordCount(Buffer.from(blob.content, blob.encoding || 'base64').toString('utf8'));
      counts.push({ name, before, after: recordCount(body) });
    }

    const gutted = counts.filter((c) => c.before !== null && c.after !== null
      && c.after < c.before - Math.max(2, Math.round(c.before * 0.1)));
    if (gutted.length && !a.force) {
      throw new RestError(
        'This publish would remove a large part of the site, so nothing was committed: '
        + gutted.map((c) => `${c.name} ${c.before} → ${c.after} records`).join(', ')
        + '. That usually means the database is missing data rather than that the data '
        + 'was deleted. Check it, then publish again with the override if it is correct.',
        409);
    }

    /* 5. One commit, so the site never deploys a half-updated dataset. */
    const blobs = [];
    for (const [name, body] of changed) {
    ...
    const commit = await gh(`/repos/${e.owner}/${e.repo}/git/commits`, {
      token: e.token, method: 'POST', body: {
        message: `Publish ${changed.length} data file${changed.length === 1 ? '' : 's'} from the desk\n\n`
          ...
          + counts.map((c) => `  data/${c.name}  ${c.before === null ? 'new' : c.before + ' → ' + c.after} records`)
              .join('\n')
