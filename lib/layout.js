...
const SUB = [
  ...
  ['franchise', 'Franchising', 'franchise-opportunities/'],
...
const FOOTER = [
  ...
  ['Real Estate', [['Real Estate Home', 'real-estate/'], ['Transactions', 'transactions/'],
  ...
  ['Company', [['About', 'about/'], ['Contact', 'contact/'], ['Advertise & Partner', 'advertise/'],
    ['Methodology', 'methodology/'], ['Privacy', 'privacy/'], ['Search', 'search/']]]
];
...
function page(opts) {
  ...
  /* The subnav scroller carries the primary links too, but only on a phone.
     Under 760px the masthead nav is hidden and the drawer was the only way to
     reach News/Brands/Operators/Real Estate/Rankings; putting them at the head
     of the scroller makes them one tap instead of two. They are marked
     .phone-only and the base stylesheet hides them, so the desktop and tablet
     subnav is unchanged. */
  const navLink = (extra) => ([k, label, href]) => {
    const cls = [extra, active === k ? 'on' : ''].filter(Boolean).join(' ');
    return `<a href="${u(href)}"${cls ? ` class="${cls}"` : ''}>${esc(label)}</a>`;
  };
  const subHtml = PRIMARY.map(navLink('phone-only')).join('') + SUB.map(navLink('')).join('');

  /* The drawer duplicates the two navs as one flat list. Under 760px the
     masthead nav is hidden and the subnav — which now carries every link,
     primary ones first — is a masked horizontal scroller whose tail only
     surfaces if you scroll it, so the drawer stays the one place a phone sees
     the whole site at once. Desktop never renders it: the base stylesheet sets
     .drawer{display:none} and only the mobile media query turns it on. */
  const drawerLink = navLink('');
