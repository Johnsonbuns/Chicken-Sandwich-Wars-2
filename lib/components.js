...
function table({ cols, rows, cls = '' }) {
  /* The identity column -- the one carrying the subject of the row rather than
     ...
     `id` may instead be a pixel width, which lowers the floor for a table where
     the default reserves more than the column needs. Worth doing wherever the
     table also carries a prose column: holding one long name to two lines
     instead of three is not worth pushing a whole column off the screen. */
  ...
  /* A leading date column has to be wide enough for its longest single token.
     CSS breaks at hyphens, so a column squeezed below that renders "2026-07" as
     "2026-" over "07", which reads as a broken table rather than a wrapped one.
     One floor for every table is the wrong instrument -- wide enough for
     "2025-06-04" turns tables that currently fit into tables that scroll -- so
     it is measured per table from the widest token actually present. Phrases
     still wrap at their spaces; only the tokens are protected. The character
     estimate is approximate, but this only ever sets a floor, so a few pixels
     either way costs nothing. */
  const firstCells = idIndex === 0 ? [] : rows.map((r) => (r.cells || r)[0] || '');
  const longestToken = firstCells.reduce((max, cell) => String(cell)
    .replace(/<[^>]*>/g, ' ').replace(/&[^;]+;/g, ' ')
    .split(/\s+/).reduce((m, tok) => Math.max(m, tok.length), max), 0);
  /* 9px per character and 28px of cell padding. Measured against the table font,
     the widest digit runs come out at 8.9px a character, so this rounds up
     rather than down -- an overshoot of a pixel or two on a floor is invisible,
     an undershoot puts a hyphen on the next line. */
  const firstWidth = longestToken ? Math.min(longestToken * 9 + 28, 140) : 0;

  /* Carried on the wrapper, not the <table> element: several pages bolt on
     sortability with .replace('<table>', '<table data-sortable>'), which would
     silently stop matching if this tag grew an attribute. */
  const idWidth = cols.find((c) => typeof c === 'object' && typeof c.id === 'number');
  const vars = [
    idWidth ? `--idw:${idWidth.id}px` : '',
    firstWidth ? `--fcw:${firstWidth}px` : ''
  ].filter(Boolean).join(';');
  const idStyle = vars ? ` style="${vars}"` : '';
  ...
  return `<div class="tablescroll${cls ? ' ' + cls : ''}"${idStyle}>${toggle}<div class="tablewrap"><table><thead><tr>${head}</tr></thead><tbody>${body}</tbody></table></div><span class="tablefade" aria-hidden="true"></span>${hint}</div>`;
}
...
/* Freshness of a scoring input, in the badge vocabulary the rest of the site uses.
   'current' gets no badge: the absence of a warning is the statement, and badging the
   normal case would train the eye to skip the abnormal one. */
const FRESH_BADGE = { aging: 'warn', stale: 'bad', unknown: 'mut' };

function freshnessBadge(state, label) {
  const cls = FRESH_BADGE[state];
  if (!cls) return '';
  return `<span class="badge ${cls}">${esc(label || state)}</span>`;
}

/**
 * The score components, each with the as-of date of the figure behind it.
 *
 * `fresh` is the brandFreshness() result. It is optional so a caller that has not got
 * one still renders — but where it is passed, every bar states the period its number
 * comes from, which is the whole editorial rule applied to the rankings themselves.
 */
function scoreBars(parts, fresh) {
  const by = new Map(((fresh && fresh.items) || []).map((it) => [it.key, it]));
  return `<div class="bars">${parts.map((p) => {
    const f = by.get(p.key);
    const asOf = f
      ? `<span class="asof ${f.state}">${esc(f.asOf || 'undated')}</span>`
      : '';
    return `<div class="bar">
    <div class="lbl">${esc(p.label)}${asOf}</div>
...
function form({ id, title, intro, fields, submit }) {
  ...
  return `<div class="card" style="padding:24px">
    ...
    <form class="csw" id="${esc(id)}" data-csw-form>
      ${html}
      <div class="hp" aria-hidden="true"><label for="${esc(id)}-website">Website</label><input id="${esc(id)}-website" name="website" type="text" tabindex="-1" autocomplete="off"></div>
      <div><button class="btn" type="submit">${esc(submit)}</button></div>
      <p class="formnote" data-formnote>${id === 'newsletter'
        ? 'One email a week. We confirm your address first, and you can unsubscribe from any issue. See the <a href="../privacy/">privacy policy</a>.'
        : 'Submitting sends these details to the CSW desk. Nothing here is published. See the <a href="../privacy/">privacy policy</a>.'}</p>
    </form>
  </div>`;
}
...
module.exports = {
  ...
  scoreBars, freshnessBadge, newsItem, emptyState, form, MOMENTUM
};
