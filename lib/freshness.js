 ...
 *
 * These are not guesses, and they are not a second opinion either: they are the same
 * correspondence `scripts/db-import.js` uses in the other direction. That importer writes
 * each scoring fact with `period(stat)` — auv_usd from the `auv` stat, comps_pct from
 * `compsLatest`, unit_growth_pct from the unit count it was derived from, sales_growth_pct
 * from systemwide sales, cap_rate_mid from realEstate.capSrc. Reading the date back out
 * along the same path is what makes the fallback sound rather than a heuristic, so the two
 * lists have to stay in step: change one and change the other.
 *
...
const MIRRORS = {
  ...
  expansion:  ['usUnits', 'globalUnits', 'usCanadaUnits', 'currentUnits', /^u(s|nits)/],
